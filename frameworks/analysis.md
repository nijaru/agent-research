...
## Ion's Multi-Agent Architecture (Cross-Reference)

**Repo:** /Users/nick/github/nijaru/ion
**Status:** Phase 1 Pi Parity ~90%, Phase 2 multi-agent design complete

Ion has the deepest analysis of Pi, Codex, and Claude Code internals available.

### What Ion Found

**In Pi's source** (742-line agent-loop.ts):
- Pure turn sequencing, deterministic event emission
- Three layers: agent-loop.ts → types.ts → agent.ts
- PendingMessageQueue for steering/follow-up

**In Claude Code's source:**
- Partitioned tool execution (read-only vs non-read-only)
- Agent Teams: shared task list, mailbox, hooks (TeammateIdle, TaskCreated, TaskCompleted)

**In Codex's source:**
- SQLite-backed threads, auto-compaction, pre-sampling compaction
- TOML-based agent definitions, CSV batch fan-out

### Ion's Design Decisions (Applicable to Pi)

| Decision | Ion's Choice | Rationale |
|----------|-------------|-----------|
| Channel model | Intercom-style async (send, ask/reply, broadcast) | Most flexible |
| Agent lifetime | Task-scoped, session-scoped, mission-scoped | Different tasks need different lifetimes |
| Context isolation | Fresh by default, fork on demand | Cost control |
| Bounded depth | Default 3, configurable to 5 | Deep enough for complex decomposition |
| Quality gates | BeforeSpawn, AfterTask, OnIdle, OnEscalate | Hooks at key lifecycle points |
| Model routing | Explicit, role-based, or auto (cost-aware) | 20-25% cost reduction from Factory Router |
| Communication | Hub-and-spoke → peer-to-peer evolution | Start simple, add complexity |

### Ion's Comparison Matrix

| Feature | Pi | Claude Code | Codex | Cursor | Droid | Amp |
|---------|-----|-------------|-------|--------|-------|-----|
| Subagent spawning | ✅ | ✅ | ✅ | ✅ | ✅ | ✅ |
| Bidirectional messaging | ✅ intercom | ✅ teams | ❌ | ❌ | ❌ | ❌ |
| Persistent agents | ✅ | ✅ | ❌ | ❌ | ✅ | ❌ |
| Mid-flight steering | ✅ | ✅ | ❌ | ❌ | ❌ | ❌ |
| Nested spawning | ✅ | ✅ depth 5 | ✅ depth 1 | ✅ | ✅ | ❌ |
| Quality gates | ✅ hooks | ✅ hooks | ✅ approval | ✅ judge | ✅ validation | ✅ |
...
- /Users/nick/github/nijaru/ion/ai/design/multi-agent-architecture-2026-06.md
- /Users/nick/github/nijaru/ion/ai/review/pi-agent-loop-deep-dive.md
